...
class Checkout extends MY_Controller 
{
    ...
    public function index($input = null) 
    {
        ...
        $data['cart'] = $this->checkout->select([
            'cart.id', 'cart.qty', 'cart.subtotal', 'product.title', 'product.image',
            'product.price' 
        ])
        ->join('product')
    ...
    public function create() 
{
    ...
    // Perbaikan: total default ke 0 jika null
    $row = $this->db->select_sum('subtotal')
        ->where('id_user', $this->id)
        ->get('cart')
        ->row();

    $total = isset($row->subtotal) ? $row->subtotal : 0;

    // Cegah order jika total = 0
    if ($total <= 0) {
        $this->session->set_flashdata('warning', 'Total belanja tidak valid.');
        return $this->index($input);
    }
